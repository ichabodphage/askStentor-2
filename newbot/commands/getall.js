...
module.exports = {
    ...
    */
    data: new SlashCommandBuilder()
        .setName('getall')
        .setDescription('displays all the term catagories')
        .addStringOption(option => option.setName('catagory').setDescription('optional, enter in a name of a catagory to brose the catagory')),
    async execute(interaction) {
        var catInput = await interaction.options.getString('catagory');
        if (catInput === null || catInput === undefined) {
            ...
            console.log(await interaction.options)
            ...
            const filter = v => v.user.id === interaction.user.id;
            const collector = interaction.channel.createMessageComponentCollector({ filter, time: 32000 });
            ...
            collector.on('end',async function(collected){
                await interaction.channel.send("menu timed out")
            });
        }
    },
};